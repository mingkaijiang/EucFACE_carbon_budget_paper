make_micc_treatment_abs_effect_statistics <- function(inDF, 
                                                      var.col, 
                                                      return.outcome) {
    
    ### subset pre-treatment data
    preDF <- subset(inDF, Date=="2012-06-13")
    inDF <- subset(inDF, Date>="2012-10-26")
    for (i in 1:6) {
        inDF$PreTrt[inDF$Ring==i] <- preDF$microbial_pool[preDF$Ring==i]
    }
    ...
    tDF <- summaryBy(Value+Cov2~Trt+Ring+Datef,data=inDF,FUN=mean, keep.names=T)
