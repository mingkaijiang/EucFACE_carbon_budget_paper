#- Make the DBH data
make_dbh_and_wood_stock <- function(include.bark.effect){
    ...
    if (include.bark.effect==T) {
        ...
        data$wood_stock <- data$biom * c_fraction * 1000
        
        #- format dataframe to return
        out <- data[,c("Date","Ring","Classif","Height","diam",
                       "wood_stock")]
    } else {
        #- convert from kg DM to g C 
        long$wood_stock <- long$biom * c_fraction * 1000
        
        #- format dataframe to return
        out <- long[,c("Date","Ring","Classif","Height","diam",
                       "wood_stock")]
    }
    
    colnames(out) <- c("Date", "Ring", "Class", "Height", "Diameter", "Wood_Stock")
